...
class LatLong {
  ...
  constructor(sentence: string) {
    ...
    this.accuracy =
      (this.lastHorizontalAccuracy + this.lastVerticalAccuracy) / 2;
  }

  private initRMC(l: string[]): void {
    const mandatoryIndices = [1, 2, 3, 4, 5, 6, 7, 9, 12];
    if (mandatoryIndices.some(index => !l[index])) {
      return;
    }
  ...
  static significantChange(oldLocation: LatLong, newLocation: LatLong) {
    ...
    return (
      !oldLocation ||
      Math.abs(oldLocation.latitude - newLocation.latitude) >
        ANGLE_SIGNIFICANT_DIFF ||
